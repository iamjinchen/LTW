...
def get_same_padding_conv2d_meta(image_size=None):
    """Chooses static padding if you have specified an image size, and dynamic padding otherwise.
       Static padding is necessary for ONNX exporting of models.
    ...
    """
    if image_size is None:
        return MetaConv2dDynamicSamePadding
    else:
        return partial(MetaConv2dStaticSamePadding, image_size=image_size)
...
class MetaConv2dDynamicSamePadding(MetaModule):
    """2D Convolutions like TensorFlow, for a dynamic image size.
       The padding is operated in forward function by calculating dynamically.
    """
    ...
    def forward(self, x):
        ...
        if pad_h > 0 or pad_w > 0:
            x = F.pad(x, [pad_w // 2, pad_w - pad_w // 2, pad_h // 2, pad_h - pad_h // 2])
        #return Conv2dDynamicSamePadding(x, self.weight, self.bias, self.stride, self.padding, self.dilation, self.groups)
        return F.conv2d(x, self.weight, self.bias, self.stride, self.padding, self.dilation, self.groups)
...
class MetaConv2dStaticSamePadding(MetaModule):
    """2D Convolutions like TensorFlow's 'SAME' mode, with the given input image size.
       The padding mudule is calculated in construction function, then used in forward.
    """
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.in_channels = ignore.in_channels
        self.out_channels = ignore.out_channels
        self.stride = ignore.stride if len(ignore.stride) == 2 else [ignore.stride[0]] * 2
        self.padding = ignore.padding
        self.dilation = ignore.dilation
        self.groups = ignore.groups
        self.kernel_size = ignore.kernel_size
