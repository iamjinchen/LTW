...
def train_main(gpu):
    ...
    if gpu==0 and os.path.exists(save_dir):
    ...
    print('model create success on {}!!!'.format(gpu))
    model.cuda(gpu)#need to load to gpu before DistributedDataParallel
    model= nn.parallel.DistributedDataParallel(model.model, device_ids=[gpu]
    ...
    criterion_oc = CompactLoss().cuda(gpu)
    ...
    df_train_sampler = torch.utils.data.distributed.DistributedSampler(df_train_dataset,
                                                                    num_replicas=world_size,
                                                                    rank=rank)
    f2f_train_sampler = torch.utils.data.distributed.DistributedSampler(f2f_train_dataset,
                                                                    num_replicas=world_size,
                                                                    rank=rank)
    fs_train_sampler = torch.utils.data.distributed.DistributedSampler(fs_train_dataset,
                                                                    num_replicas=world_size,
                                                                    rank=rank)
    nt_train_sampler = torch.utils.data.distributed.DistributedSampler(nt_train_dataset,
                                                                    num_replicas=world_size,
                                                                    rank=rank)
    ...
    cele_test_sampler = torch.utils.data.distributed.DistributedSampler(cele_test_dataset,
                                                                    num_replicas=world_size,
                                                                    rank=rank)
    dfdc_test_sampler = torch.utils.data.distributed.DistributedSampler(dfdc_test_dataset,
                                                                    num_replicas=world_size,
                                                                    rank=rank)
    df_train_sampler = torch.utils.data.distributed.DistributedSampler(df_test_dataset,
                                                                    num_replicas=world_size,
                                                                    rank=rank)
    f2f_train_sampler = torch.utils.data.distributed.DistributedSampler(f2f_test_dataset,
                                                                    num_replicas=world_size,
                                                                    rank=rank)
    fs_train_sampler = torch.utils.data.distributed.DistributedSampler(fs_test_dataset,
                                                                    num_replicas=world_size,
                                                                    rank=rank)
    nt_train_sampler = torch.utils.data.distributed.DistributedSampler(nt_test_dataset,
                                                                    num_replicas=world_size,
                                                                    rank=rank)
