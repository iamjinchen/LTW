...
def main(rank,world_size):
    ...
    if fp16:
        ...
        optimizer = FusedAdam(optimizer_grouped_parameters,
                                ...
                                max_grad_norm=1.0,
                                betas=(beta1,beta2))
        fnet_optimizer = FusedAdam(fnet_optimizer_grouped_parameters, lr=plr, bias_correction=False,
                                max_grad_norm=1.0,betas=(beta1,beta2))
        if loss_scale == 0:
            optimizer = FP16_Optimizer(optimizer, dynamic_loss_scale=True)
            optimizer = FP16_Optimizer(fnet_optimizer, dynamic_loss_scale=True)
        else:
