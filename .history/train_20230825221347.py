...
from utils import *
import time
os.environ['CUDA_VISIBLE_DEVICES']='0,1,2,3'#important
...
def train_main(gpu):
    ...
    dfdc_test_dataset = DFDCDetection(root = dfdc_path, train=False, frame_nums=frame_nums, transform=_preproc,split_path=dfdc_data_list)
    
    cele_test_sampler = torch.utils.data.distributed.DistributedSampler(cele_test_dataset,
                                                                    num_replicas=world_size,
                                                                    rank=rank)
    df_test_sampler = torch.utils.data.distributed.DistributedSampler(df_test_dataset,
                                                                    num_replicas=world_size,
                                                                    rank=rank)
    f2f_test_sampler = torch.utils.data.distributed.DistributedSampler(f2f_test_dataset,
                                                                    num_replicas=world_size,
                                                                    rank=rank)
    fs_test_sampler = torch.utils.data.distributed.DistributedSampler(fs_test_dataset,
                                                                    num_replicas=world_size,
                                                                    rank=rank)
    nt_test_sampler = torch.utils.data.distributed.DistributedSampler(nt_test_dataset,
                                                                    num_replicas=world_size,
                                                                    rank=rank)
    dfdc_test_sampler = torch.utils.data.distributed.DistributedSampler(nt_test_dataset,
                                                                    num_replicas=world_size,
                                                                    rank=rank)
    
    df_test_dataloader = data.DataLoader(df_test_dataset, batch_size=2, shuffle=False, num_workers=8,sampler=df_test_sampler)
    f2f_test_dataloader = data.DataLoader(f2f_test_dataset, batch_size=2, shuffle=False, num_workers=8,sampler=f2f_test_sampler)
    fs_test_dataloader = data.DataLoader(fs_test_dataset, batch_size=2, shuffle=False, num_workers=8,sampler=fs_test_sampler)
    nt_test_dataloader = data.DataLoader(nt_test_dataset, batch_size=2, shuffle=False, num_workers=8,sampler=nt_test_sampler)
    cele_test_dataloader = data.DataLoader(cele_test_dataset, batch_size=2, shuffle=False, num_workers=8,sampler=cele_test_sampler)#shuffle=true
    dfdc_test_dataloader = data.DataLoader(dfdc_test_dataset, batch_size=3, shuffle=False, num_workers=8, sampler=dfdc_test_sampler)#shuffle=true
    ...
    for epoch in range(epochs):
        ...
        train_sampler = torch.utils.data.distributed.DistributedSampler(train_dataset,
                                                                    num_replicas=world_size,
                                                                    rank=rank)
        train_dataloader = data.DataLoader(train_dataset, batch_size=batch_size, shuffle=False, num_workers=8,worker_init_fn=worker_init_fn,sampler=train_sampler)#shuffle=true
...
def main():
    ...
    mp.spawn(train_main, nprocs=gpus)    
