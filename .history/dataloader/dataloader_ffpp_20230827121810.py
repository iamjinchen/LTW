...
class FFpp(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.pair == True:
            ...
            if target == 1:
                ...
                #find the real frame of that first id
                opposite_path = f'{self.original_path}/{self.compress}/'+origin_index+"/" + frame_index
                #find the correspond frame of its meta type
                meta_path = img_path.replace(img_path.split('/')[-4],self.meta_type)
                if not os.path.exists(opposite_path):
                    #if this image frame not found, choose random frame from this id
                    original_folder = f'{self.original_path}/{self.compress}/'+origin_index+"/"
                    face_paths = glob.glob(os.path.join(original_folder, '*.png'))
                    opposite_path = random.choice(face_paths)
                if not os.path.exists(meta_path):
                    original_folder = f'{self.fake_path}/{self.meta_type}/{self.compress}/'+img_path.split('/')[-2]+"/"
                    face_paths = glob.glob(os.path.join(original_folder, '*.png'))
            ...
            if target == 0:
                ...
                opposite_path = f'{self.fake_path}/{self.meta_type}/{self.compress}/'+fake_index+"/"+frame_index
                if not os.path.exists(opposite_path):
                    original_folder = f'{self.fake_path}/{self.meta_type}/{self.compress}/'+fake_index+"/"
                    face_paths = glob.glob(os.path.join(original_folder, '*.png'))
                    opposite_path = random.choice(face_paths)
                if not os.path.exists(meta_path):
                    original_folder = f'{self.original_path}/{self.compress}/'+target_index+"/"
                    face_paths = glob.glob(os.path.join(original_folder, '*.png'))
                    meta_path = random.choice(face_paths)
    ...
    def init(self):
        ...
        if self.type != 'all' and self.type!='real':
            Deepfakes_path = f'{self.fake_path}/{self.type}/{self.compress}/'
        ...
        original_path = f'{self.original_path}/{self.compress}/'
        path_list.append(original_path)
        self.num_real = 0
        self.num_fake = 0
        for path in path_list:

            label_str = path.split('/')[-4]
            ...
            for folder in folder_paths:
                ...
                # return image_path, label, the folder of images
...
if __name__ == '__main__':
    ...
    f2f_train_dataset = FFpp(split='test', frame_nums=frame_nums, transform=_preproc,detect_name = detect_name,compress = compress,type = 'Face2Face',pair = True,original_path=ffpp_original_path,fake_path=ffpp_fake_path)
    ...
    # df_test_dataset = FFpp(split='test', frame_nums=frame_nums, transform=_preproc,detect_name = detect_name,compress = compress,type = "Deepfakes",pair = True,original_path=ffpp_original_path,fake_path=ffpp_fake_path)
    # f2f_test_dataset = FFpp(split='test', frame_nums=frame_nums, transform=_preproc,detect_name = detect_name,compress = compress,type = 'Face2Face',pair = True,original_path=ffpp_original_path,fake_path=ffpp_fake_path)
    # fs_test_dataset = FFpp(split='test', frame_nums=frame_nums, transform=_preproc,detect_name = detect_name,compress = compress,type = 'FaceSwap',pair = True,original_path=ffpp_original_path,fake_path=ffpp_fake_path)
    # nt_test_dataset = FFpp(split='test', frame_nums=frame_nums, transform=_preproc,detect_name = detect_name,compress = compress,type = 'NeuralTextures',pair = True,original_path=ffpp_original_path,fake_path=ffpp_fake_path)
