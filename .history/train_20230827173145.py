...
from transformers import get_linear_schedule_wit
_warmup
...
def train(model,optimizer,warmup_linear,fnet,optimizer_fnet,fnet_warmup_linear,train_dataloader,meta_dataloader,criterion_oc,epoch,epoch_size,device):
    '''
    ...
    for i, datas in enumerate(train_dataloader):
        ...
        if fp16:
            optimizer.backward(loss_add)
            fnet.backward(loss_add)
        else:
            loss_add.backward()
        optimizer.step()
        warmup_linear.setp()
        optimizer_fnet.step() 
        fnet_warmup_linear.step()    
...
def main(rank,world_size):
    ...
    for epoch in range(epochs):
        ...
        train(model,optimizer,warmup_linear,fnet,optimizer_fnet,fnet_warmup_linear,train_dataloader,None,criterion_oc,epoch,epoch_size,device)
